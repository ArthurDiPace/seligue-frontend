...
export default {
  ...
  methods: {
    ...
    async getEquipamentos() {
      ...
      const [equipamentosResponse, veiculosResponse] = await Promise.all([
        this.$api.list({
      ...
      const equipamentos = this.toUpperCaseItems(equipamentosResponse.data.results);
      const veiculos = this.toUpperCaseItems(veiculosResponse.data.results);

      this.items = [...equipamentos, ...veiculos];
      this.totalItems = equipamentosResponse.data.count + veiculosResponse.data.count;
    },
    async cadastrarEquipamento() {
      try {
        if (this.categoria === 'veiculo') {
          this.$router.push({ name: 'veiculo/VeiculoCreate' });
        } else {
          this.$router.push({ name: 'equipamento/EquipamentoCreate' });
        }
    ...
    editarEquipamento(id) {
      const equipamento = this.items.find(item => item.id === id);
  
      if (equipamento && equipamento.categoria === 'veiculo') {
        this.$router.push({ name: 'veiculo/VeiculoEdit', params: { id: id } });
      } else {
        this.$router.push({ name: 'equipamento/EquipamentoEdit', params: { id: id } });
      }
    },
    limparFiltros() {
      this.filtro = {
